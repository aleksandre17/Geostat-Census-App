...
public class RunnableVector implements Runnable {
    ...
    public void run() {
        ...
        activity.getArcgisController().setFeatureLayersNames(geoPackageVector.getGeoPackageFeatureTables());
...
//                Log.d("TEST", "accept: " + geoPackageFeatureTable.getTableName());
        ...
        FeatureLayer line = activity.getArcgisController().createLayer("road_cl_napr");
        ...
        FeatureLayer mainFeatureLayer = activity.getArcgisController().createLayer("house_point");
        ...
        try {
            FeatureCollectionLayer maskEffectLayer = activity.getArcgisController().createLayer(userLocation);
            maskEffectLayer.getFeatureCollection()
                    .getTables().get(0).setRenderer(activity.getArcgisController().featureRender("featureCollectionTable"));
...
//        );
    ...
    private UniqueValueRenderer initMarkers() {
        ...
        Map<Integer, Integer> uniqueValueRendererMap =
                Map.ofEntries(Map.entry(0, 1), Map.entry(1, 2), Map.entry(2, 3), Map.entry(3, 4), Map.entry(4, 7));
        List<SimpleMarkerSymbol.Style> styles =
                List.of(SimpleMarkerSymbol.Style.CROSS, SimpleMarkerSymbol.Style.CROSS, SimpleMarkerSymbol.Style.DIAMOND, SimpleMarkerSymbol.Style.CROSS, SimpleMarkerSymbol.Style.DIAMOND);
        List<Integer> colors =
                List.of(Color.YELLOW, Color.GREEN, Color.RED, Color.BLACK, Color.GREEN);
